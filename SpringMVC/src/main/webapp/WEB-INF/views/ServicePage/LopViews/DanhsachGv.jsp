				...
				<c:if test="${message eq null }">
					...
					<div class="row mt-3">
						...
						<c:choose>
							<c:when test="<%=p > 1%>">
								<a
									href="/QuanLiLopMVC/lop/tim-kiem-giaovien?lopID=${maLop}&&p=<%=p - 1%>">Previous</a>
							</c:when>
							<c:otherwise>
								<a
									href="/QuanLiLopMVC/lop/tim-kiem-giaovien?lopID=${maLop}&&p=1">Previous</a>
							</c:otherwise>
						</c:choose>
						...
						<c:choose>
							<c:when test="<%=p < maxPage%>">
								<a
									href="/QuanLiLopMVC/lop/tim-kiem-giaovien?lopID=${maLop}&&p=<%=p + 1%>">Next</a>
							</c:when>
							<c:otherwise>
								<a
									href="/QuanLiLopMVC/lop/tim-kiem-giaovien?lopID=${maLop}&&p=${maxPage}">Next</a>
							</c:otherwise>
						</c:choose>
					</div>
					<!-- end Page -->
