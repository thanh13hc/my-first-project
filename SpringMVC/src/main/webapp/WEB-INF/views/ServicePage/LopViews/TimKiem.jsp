		...
		<div class="row">
			...
			<div class="col-sm-8">
				<div class="row" style="background-color: #435D7D;">
					...
					<div class="col-sm-6 mt-3 mb-3">
						<form action="/QuanLiLopMVC/lop/tim-kiem" method="post"
					...
					<c:forEach var="lop" items="${list}">
						<tbody>
							<tr>
								...
								<td>${lop.tenLop}</td>
								<td><a
									href="/QuanLiLopMVC/lop/tim-kiem-giaovien?lopID=${lop.maLop}"
									class="btn btn-outline-success"> Chọn </a></td>
								<td><a href="/QuanLiLopMVC/lop/tim-kiem-sinhvien?lopID=${lop.maLop}"
									class="btn btn-outline-success"> Chọn </a></td>
							</tr>
						</tbody>
					</c:forEach>
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<div class="row mt-3">
							...
							<c:choose>
								<c:when test="<%=p > 1%>">
									<a href="/QuanLiLopMVC/lop/tim-kiem?p=<%=p - 1%>">Previous</a>
								</c:when>
								<c:otherwise>
									<a href="/QuanLiLopMVC/lop/tim-kiem?p=1">Previous</a>
								</c:otherwise>
							</c:choose>
							...
							<c:choose>
								<c:when test="<%=p < maxPage%>">
									<a href="/QuanLiLopMVC/lop/tim-kiem?p=<%=p + 1%>">Next</a>
								</c:when>
								<c:otherwise>
									<a href="/QuanLiLopMVC/lop/tim-kiem?p=${maxPage}">Next</a>
								</c:otherwise>
							</c:choose>
						</div>
					</c:otherwise>
