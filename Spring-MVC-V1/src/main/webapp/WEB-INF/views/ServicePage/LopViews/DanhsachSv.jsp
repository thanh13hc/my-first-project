	...
	<div class="container-fluid mt-5">
		<div class="row">
			...
			<div class="col-sm-8">
				...
				<form action="/QuanLiLopMVC/sinhvien/deletes" method="post">
					<table class="table">
						<thead>
							<tr>
								<th scope="col">#</th>
						...
						<c:forEach var="sinhvien" items="${list}">
							<tbody>
								<tr>
									...
									<td>${sinhvien.ten}</td>
									<td><a
										href="/QuanLiLopMVC/sinhvien/form-edit?id=${sinhvien.maSinhvien}">
											<i class="far fa-edit"></i>
									</a> <a
										href="/QuanLiLopMVC/sinhvien/delete?id=${sinhvien.maSinhvien}"><i
				...
				<c:if test="${message eq null }">
						...
						<div class="row mt-3">
							...
							<c:choose>
								<c:when test="<%=p > 1%>">
									<a href="/QuanLiLopMVC/lop/tim-kiem-sinhvien?lopID=${maLop}&&p=<%=p - 1%>">Previous</a>
								</c:when>
								<c:otherwise>
									<a href="/QuanLiLopMVC/lop/tim-kiem-sinhvien?lopID=${maLop}&&p=1">Previous</a>
								</c:otherwise>
							</c:choose>
							...
							<c:choose>
								<c:when test="<%=p < maxPage%>">
									<a href="/QuanLiLopMVC/lop/tim-kiem-sinhvien?lopID=${maLop}&&p=<%=p + 1%>">Next</a>
								</c:when>
								<c:otherwise>
									<a href="/QuanLiLopMVC/lop/tim-kiem-sinhvien?lopID=${maLop}&&p=${maxPage}">Next</a>
								</c:otherwise>
							</c:choose>
						</div>
						<!-- end Page -->
